...
def preprocessing(samples, labels, n_samples_per_class):
    """ 
    Balance classes and permute shape
    """
    ...
    for i in range(0, 10):
        ...
        if (len(idx[0]) - n_samples_per_class) > 0:
            ...
            raise Exception('MNISTM.py_ABORT -> not enoguh samples per class')
...
def save_random_samples(samples, labels):
    ...
    plt.savefig('results/dataset_samples/mnistm.png')
    if VERBOSE:
        plt.show()
    plt.close()
...
class MNISTM(Dataset):
    def __init__(self, split: str = 'test', test_size = 0.20, n_samples_per_class = 300):
        ...
        # --- Load MNISTM-M Samples ---
        with open('data/original_dataset_files/mnistm_samples.pkl', 'rb') as f:
            mnistm = pickle.load(f, encoding='latin')
        x = np.uint8(mnistm.get('train'))
        # -----------------
        
        #put the shape equal to SVHN
        x = np.transpose(x, (1,2,3,0))

        # --- Load MNISTM-M Labels ---
        with gzip.open('data/original_dataset_files/mnistm_labels.gz', 'rb') as f:
        ...
        if split == 'test':
            ...
            if VERBOSE:
                print('MNISTM.PY - Test Dataset Dimensions:')
            ...
            save_random_samples(self.samples, self.labels)
            if VERBOSE:
                print('MNISTM.PY - Train Dataset Dimensions:')
                print('samples.shape:', self.samples.shape, 'labels.shape:', len(self.labels))
